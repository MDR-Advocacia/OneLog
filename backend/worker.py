...
def processar_login(setor):
    ...
    with SB(uc=True, test=True, headless=True, proxy="socks5://206.42.43.192:45123", agent=USER_AGENT_DOURADO) as sb:
        try:
            ...
            # 4. Verificação de Token (Faltava no worker anterior!)
            ...
            # Salva o Cookie no Redis com Timestamp (Válido por 30min no BB, nós renovamos com 20)
            cookie_payload = {
                "created_at": datetime.now().timestamp(),
                "cookies": cookies
            }
            redis_client.set(f"cookie:{setor}", json.dumps(cookie_payload))
...
if __name__ == "__main__":
    logger.info("Worker iniciado. Aguardando tarefas na fila 'queue:login'...")
    while True:
        # Puxa o próximo setor da fila (Bloqueia até chegar algo)
        _, setor = redis_client.brpop("queue:login")
        logger.info(f"Nova tarefa recebida para o setor: {setor}")
        processar_login(setor)